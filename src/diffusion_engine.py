...
class DiffusionEngine:
    ...
    def _worker(self) -> None:
        cfg = self.cfg
        # Limit PyTorch CPU threads in this worker — reduces GIL contention
        # with the MediaPipe pose thread which also uses CPU threadpools.
        torch.set_num_threads(2)
        generator = torch.Generator(device=cfg.device).manual_seed(42)

        # Double-buffer: while GPU runs inference on frame N+1,
        # we copy frame N from GPU to CPU on a separate stream.
        copy_stream = torch.cuda.Stream()
        prev_gpu_frame: torch.Tensor | None = None  # (1,3,H,W) float16 on GPU

        while self._running:
            # Drain queue — keep only the freshest control map
            control_map = None
            try:
                while True:
            ...
            # ── GPU inference (output stays on GPU as float16 tensor) ─────
            with torch.inference_mode():
                result = self._pipe(
                    prompt_embeds=self._prompt_embeds,
                    negative_prompt_embeds=self._neg_embeds,
            ...
            gpu_frame = result.images  # (1,3,H,W) float16 on GPU
